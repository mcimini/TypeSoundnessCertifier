...
let testsAsRules_byReductionRuleEliminator elimBool errorBool subtyping listConstructorsSpec typingRule reductionRule = 
	...
	let typeOfTheEliminated = formula_getFirstOutput (List.hd premisesOfTypingRule) in 
	let triplesFirst = (List.map (premiseTransformation (rule_getPremises typingRule) applicationsInConclusion) premisesOfTypingRule) in 
	let premisesFirst = List.concat (List.map get3_1 triplesFirst) in 
	let checksFirst = List.concat (List.map get3_2 triplesFirst) in 
	...
	let typingRuleEliminatedButNotSynched = specTerm_getTyping termSpecConstructor in (* Here the eliminating arguments may have type like 'E1 : (list T1)', but then the typing rule for the intro form may be (cons E1 E2) : list T. so we need to synch/substitutute T1 => T *)
	let typeOfTheEliminatedWithIsOwnTypingRule = rule_getOutputTerm typingRuleEliminatedButNotSynched in 	
	let associationsTypeElim = try List.combine (term_getArguments typeOfTheEliminatedWithIsOwnTypingRule) (term_getArguments typeOfTheEliminated) with _ -> [] in 	
	let typingRuleEliminated = rule_substitutions associationsTypeElim typingRuleEliminatedButNotSynched in 	
