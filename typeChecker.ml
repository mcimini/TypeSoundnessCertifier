	...
	(*
		...
		*)
				| Elim c -> 
					...
					ckIf 
						( ck (bindingTyp_sameTypeValue bindingsTyp op2 c) (op ^ " is an elimination form for type " ^ c ^ ", but has a reduction rule to handle " ^ op2 ^ ", which is not a value of type " ^ c) &&
				 		  ck (List.mem 1 (progressingArguments_are_contextual tl rule)) ("The principal argument of the elimination form " ^ op ^ " is not declared as evaluation context, hence some programs may get stuck"))
						(op, Eliminates op2) 
...
(*		ck (List.for_all (fun el -> List.mem el itsEliminators) eliminators) ("The value " ^ oneValue ^ " of type " ^ typ ^ " is not eliminated by all eliminators") in *)
		ck (List.for_all (fun el -> List.mem el itsEliminators) eliminators)  ("Elimination form " ^ String.concat ", " (list_difference eliminators itsEliminators) ^ " does not have a reduction rule for handling " ^ oneValue) in 
